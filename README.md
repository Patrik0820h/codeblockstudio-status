...
A self-hosted, self-checking status page for [CodeBlock Studio](https://www.codeblockstudio.net), free and independent of the infrastructure it monitors.
...
3. Repo → Settings → Pages → Custom domain: `status.codeblockstudio.net` (the `CNAME` file here already has this, GitHub should pick it up automatically).
4. At whichever DNS provider hosts `codeblockstudio.net`, add a CNAME record: Name `status`, Value `<your-github-username>.github.io`.
